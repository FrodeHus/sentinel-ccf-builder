...
import { generateArmTemplate } from "./arm-generator"
...
export function downloadArmTemplate(config: ConnectorConfig) {
  const template = generateArmTemplate(config)
  downloadJson(template, "mainTemplate.json")
}

export function downloadIndividualFile(type: "table" | "dcr" | "connectorDef" | "dataConnector", config: ConnectorConfig) {
  ...
  switch (type) {
    ...
    case "dcr":
      ...
      break
    case "connectorDef":
      downloadJson(generateConnectorDefinition(meta, schema, connectorUI), "connectorDefinition.json")
      break
    case "dataConnector":
...
export async function downloadSolutionZip(config: ConnectorConfig) {
  ...
  const dcFolder = root.folder("Data Connectors")!.folder(`${connectorName}_ccf`)!
  dcFolder.file("table.json", JSON.stringify(generateTableResource(schema, ""), null, 2))
  dcFolder.file("DCR.json", JSON.stringify(generateDcrResource(schema, dataFlow, dcrName), null, 2))
  dcFolder.file("connectorDefinition.json", JSON.stringify(generateConnectorDefinition(meta, schema, connectorUI), null, 2))
  dcFolder.file("dataConnector.json", JSON.stringify(generateDataConnector(meta, dataFlow), null, 2))
  ...
  dataFolder.file(`Solution_${connectorName}.json`, JSON.stringify(solutionMetadata, null, 2))

  // SolutionMetadata.json
  root.file("SolutionMetadata.json", JSON.stringify({
    publisherId: solution.publisherId,
    offerId: solution.offerId,
  ...
  // ReleaseNotes.md
  root.file("ReleaseNotes.md", `# ${meta.title}\n\n## v${solution.version}\n\n- Initial release\n`)

  // mainTemplate.json
  root.file("mainTemplate.json", JSON.stringify(generateArmTemplate(config), null, 2))
